...
async function getPageOpts(c: any) {
  ...
  return {
    ...
    storeSettings,
    // COD_MODE: 'manual' (default) | 'cashfree'
    codMode: (storeSettings.COD_MODE || 'manual') as string,
  };
}
...
app.post('/api/checkout/cod', async (c) => {
  try {
    ...
    // ============ OTP GUARD (when COD_MODE = 'cashfree') ============
    // Check store setting — only enforce OTP guard in Cashfree mode
    const codMode = await fetchStoreSetting(sbUrl, sbKey, 'COD_MODE') || 'manual';
    if (codMode === 'cashfree' && sbUrl && sbKey) {
      const normalizedPhone = userPhone.toString().trim().replace(/^91/, '');
      const sessionRes = await supabaseFetch(
        sbUrl, sbKey,
        `cod_otp_sessions?phone=eq.${normalizedPhone}&verified=eq.true&created_at=gte.${new Date(Date.now() - 15 * 60 * 1000).toISOString()}&limit=1`
      );
      let sessions: any[] = [];
      if (sessionRes.ok) sessions = await sessionRes.json();
      if (!sessions.length) {
        return c.json({ error: 'OTP verification required. Please verify your phone number first.' }, 403);
      }
    }
    ...
    if (sbUrl && writeKey) {
      try {
        const orderPayload: any = {
          ...
          cod_mode: body.codMode || 'manual',
          rto_risk_level: body.rtoScore !== undefined
            ? (body.rtoScore <= 40 ? 'low' : body.rtoScore <= 70 ? 'medium' : 'high')
            : 'unknown',
    ...
    // Delete OTP session after successful order (one-time use)
    if (sbUrl && sbKey) {
      const normalizedPhone = userPhone.toString().trim().replace(/^91/, '');
      await supabaseFetch(sbUrl, sbKey, `cod_otp_sessions?phone=eq.${normalizedPhone}`, { method: 'DELETE' }).catch(() => { });
    }
...
// ============ CASHFREE: Send OTP for COD verification ============

app.post('/api/payment/cod/send-otp', async (c) => {
  try {
    const { phone } = await c.req.json();
    if (!phone || !/^[0-9]{10}$/.test(phone.toString().trim()))
      return c.json({ success: false, error: 'Valid 10-digit phone number required' }, 400);

    const normalizedPhone = phone.toString().trim();
    const phoneWithCC = `91${normalizedPhone}`;

    if (!c.env.CASHFREE_APP_ID || !c.env.CASHFREE_SECRET_KEY)
      return c.json({ success: false, error: 'Cashfree not configured' }, 500);

    // Call Cashfree OTP API
    const cfRes = await cashfreeRequest(c.env, '/verification/offline-verification/otp', {
      phone: phoneWithCC,
    });
    if (cfRes?.status === 'ERROR' || cfRes?.code === 'ERROR') {
      console.error('Cashfree OTP send error:', cfRes);
      return c.json({ success: false, error: cfRes?.message || 'Failed to send OTP' }, 400);
    }

    // Insert/replace OTP session in Supabase (verified=false)
    const sbUrl = getEnv(c.env, 'SUPABASE_URL');
    const sbKey = getEnv(c.env, 'SUPABASE_SERVICE_KEY') || getEnv(c.env, 'SUPABASE_ANON_KEY');
    if (sbUrl && sbKey) {
      // Delete any old session for this phone first (clean slate)
      await supabaseFetch(sbUrl, sbKey, `cod_otp_sessions?phone=eq.${normalizedPhone}`, { method: 'DELETE' });
      await supabaseFetch(sbUrl, sbKey, 'cod_otp_sessions', {
        method: 'POST',
        body: JSON.stringify({ phone: normalizedPhone, verified: false, created_at: new Date().toISOString() }),
      });
    }

    return c.json({ success: true, message: `OTP sent to ${normalizedPhone.slice(0, 6)}XXXX` });
  } catch (e: any) {
    console.error('send-otp error:', e);
    return c.json({ success: false, error: e.message || 'Failed to send OTP' }, 500);
  }
})

// ============ CASHFREE: Verify OTP + RTO check ============

app.post('/api/payment/cod/verify-otp', async (c) => {
  try {
    const { phone, otp, orderAmount, pincode, email } = await c.req.json();
    if (!phone || !otp)
      return c.json({ verified: false, error: 'Phone and OTP are required' }, 400);

    const normalizedPhone = phone.toString().trim();
    const phoneWithCC = `91${normalizedPhone}`;

    if (!c.env.CASHFREE_APP_ID || !c.env.CASHFREE_SECRET_KEY)
      return c.json({ verified: false, error: 'Cashfree not configured' }, 500);

    // Verify OTP with Cashfree
    const cfRes = await cashfreeRequest(c.env, '/verification/offline-verification/otp/verify', {
      phone: phoneWithCC,
      otp: otp.toString().trim(),
    });

    if (cfRes?.status === 'ERROR' || cfRes?.verified === false || cfRes?.code === 'ERROR') {
      return c.json({ verified: false, error: cfRes?.message || 'Invalid OTP' });
    }

    // OTP verified — update session in Supabase
    const sbUrl = getEnv(c.env, 'SUPABASE_URL');
    const sbKey = getEnv(c.env, 'SUPABASE_SERVICE_KEY') || getEnv(c.env, 'SUPABASE_ANON_KEY');
    if (sbUrl && sbKey) {
      await supabaseFetch(sbUrl, sbKey, `cod_otp_sessions?phone=eq.${normalizedPhone}`, {
        method: 'PATCH',
        body: JSON.stringify({ verified: true }),
      });
    }

    // Run Cashfree RTO Intelligence check
    const rtoAmount = typeof orderAmount === 'number' ? orderAmount : 999;
    const { score, allowed } = await checkRTORisk(c.env, {
      orderAmount: rtoAmount,
      phone: normalizedPhone,
      email: email || 'noreply@intru.in',
      pincode: pincode || '500001',
    });

    return c.json({ verified: true, rto_score: score, cod_allowed: allowed });
  } catch (e: any) {
    console.error('verify-otp error:', e);
    return c.json({ verified: false, error: e.message || 'Verification failed' }, 500);
  }
})

// ============ CASHFREE PREPAID: Fallback when Razorpay fails ============

app.post('/api/payment/prepaid/cashfree', async (c) => {
  try {
    const body = await c.req.json();
    const userEmail = body.userEmail || 'noreply@intru.in';
    const userName = body.userName || '';
    const userPhone = body.userPhone || '';
    const items = body.items || [];
    ...
    if (!c.env.CASHFREE_APP_ID || !c.env.CASHFREE_SECRET_KEY)
      return c.json({ error: 'Cashfree not configured' }, 500);
    ...
    for (const item of items) {
      ...
      validatedItems.push({
        productId: product.id, name: product.name, size: item.size,
        quantity: qty, unitPrice: product.price, lineTotal: product.price * qty,
        slug: product.slug,
      });
    }
    ...
    const cfOrderId = `INTRU_${Date.now()}`;
    const cfBase = c.env.CASHFREE_ENV === 'production' ? 'https://api.cashfree.com' : 'https://sandbox.cashfree.com';
    ...
    const cfRes = await fetch(`${cfBase}/pg/orders`, {
      ...
      body: JSON.stringify({
        ...
        order_meta: { payment_methods: 'upi,cc,dc,nb,wallet' }, // no COD
      }),
    });

    if (!cfRes.ok) {
      const errText = await cfRes.text();
      console.error('Cashfree prepaid order failed:', errText);
      return c.json({ error: 'Cashfree order creation failed' }, 500);
    }
    ...
    if (sbUrl && sbSvc) {
      try {
        const res = await supabaseFetch(sbUrl, sbSvc, 'orders', {
          method: 'POST',
          body: JSON.stringify({
            cf_order_id: cfOrderId,
            items: validatedItems, subtotal, shipping, total,
            customer_name: userName, customer_email: userEmail, customer_phone: userPhone,
            status: 'pending', payment_method: 'prepaid', cod_mode: 'cashfree_prepaid',
            created_at: new Date().toISOString(),
          }),
        });
        if (res.ok) {
      ...
      } catch (e) { console.error('CF prepaid DB error:', e); }
    }

    return c.json({ success: true, orderId, order_id: cfOrderId, payment_session_id: paymentSessionId, total, cfEnv: c.env.CASHFREE_ENV || 'sandbox' });
  } catch (e: any) {
    return c.json({ error: e.message || 'Cashfree prepaid failed' }, 500);
  }
})

// ============ CHECKOUT: COD via Cashfree Partial Payment (RTO Guard) ============
// This endpoint creates a Cashfree order for the COD Service Fee only.
// Customer pays Rs.COD_FEE upfront via Cashfree to confirm intent.
// On payment success (webhook), the order status changes from 'pending' -> 'placed'.

app.post('/api/checkout/cod-cashfree', async (c) => {
  try {
    const body = await c.req.json();
    const items = body.items;
    const userEmail = body.userEmail || '';
    ...
    if (!items || !Array.isArray(items) || items.length === 0)
      return c.json({ error: 'No items in cart' }, 400);
    if (!userName || !userPhone || !address.pincode || !address.line1)
      return c.json({ error: 'Name, phone, pincode, and address are required' }, 400);
    ...
    const cfAppId = getEnv(c.env, 'CASHFREE_APP_ID');
    const cfSecret = getEnv(c.env, 'CASHFREE_SECRET_KEY');
    const cfEnv = getEnv(c.env, 'CASHFREE_ENV') || 'sandbox';   // 'sandbox' | 'production'

    if (!cfAppId || !cfSecret)
      return c.json({ error: 'Cashfree not configured. Add CASHFREE_APP_ID & CASHFREE_SECRET_KEY to Cloudflare secrets.' }, 500);
    ...
    for (const item of items) {
      ...
      if (!product.inStock) return c.json({ error: `${product.name} is out of stock` }, 400);
      if (!item.size || !product.sizes.includes(item.size))
        return c.json({ error: `Size "${item.size}" not available` }, 400);
      const qty = Math.max(1, Math.min(10, parseInt(item.quantity) || 1));
      const lineTotal = product.price * qty;
      subtotal += lineTotal;
      validatedItems.push({
    ...
    // Fetch COD fee from store settings (default Rs.99)
    const codFee = parseInt(await fetchStoreSetting(sbUrl, sbKey, 'COD_FEE') || '99') || 99;
    const total = subtotal + codFee;

    // Generate unique order ref
    const cfOrderId = 'CF-COD-' + Date.now().toString(36).toUpperCase() + '-' + Math.random().toString(36).toUpperCase().slice(-3);

    // --- Create Cashfree Order (charge only codFee as security deposit) ---
    const cfBaseUrl = cfEnv === 'production'
      ? 'https://api.cashfree.com/pg'
      : 'https://sandbox.cashfree.com/pg';

    const cfPayload = {
      order_id: cfOrderId,
      order_amount: codFee,          // Only the security/service fee
      order_currency: 'INR',
      order_note: `COD Verification Fee for order Rs.${total} — ${validatedItems.map(i => i.name).join(', ')}`,
      customer_details: {
        customer_id: userPhone || userEmail || cfOrderId,
        customer_email: userEmail || 'noreply@intru.in',
        customer_phone: userPhone,
        customer_name: userName,
      },
      order_meta: {
        notify_url: 'https://intru.in/api/webhooks/cashfree',
      },
    };
    ...
    if (!cfRes.ok) {
      ...
      return c.json({ error: 'Cashfree order creation failed: ' + errText }, 500);
    }
      ...
      try {
        const res = await supabaseFetch(sbUrl, writeKey, 'orders', {
          ...
          body: JSON.stringify({
            ...
            items: validatedItems, subtotal, shipping: 0, total,
            customer_name: userName, customer_email: userEmail,
            customer_phone: userPhone,
            status: 'pending',
            payment_method: 'cod',
    ...
    return c.json({ error: e.message || 'Cashfree COD checkout failed' }, 500);
  }
})

// ============ CASHFREE WEBHOOK ============
// Cashfree calls this URL after the customer pays the COD service fee.
// We verify the signature, then mark the order as 'placed'.

app.post('/api/webhooks/cashfree', async (c) => {
  try {
    const rawBody = await c.req.text();
    const cfSecret = getEnv(c.env, 'CASHFREE_SECRET_KEY');
    const receivedSig = c.req.header('x-webhook-signature') || '';
    const receivedTs = c.req.header('x-webhook-timestamp') || '';

    // Verify Cashfree webhook signature (HMAC-SHA256 of timestamp+rawBody)
    if (cfSecret && receivedSig) {
      const expectedSig = await hmacSHA256(cfSecret, receivedTs + rawBody);
    ...
    const type = event.type;  // e.g. 'PAYMENT_SUCCESS_WEBHOOK'
    const data = event.data || {};
    const order = data.order || {};
    const payment = data.payment || {};

    const cfOrderId = order.order_id || '';
    if (!cfOrderId) return c.json({ status: 'ignored' });

    const sbUrl = getEnv(c.env, 'SUPABASE_URL');
    const sbKey = getEnv(c.env, 'SUPABASE_SERVICE_KEY') || getEnv(c.env, 'SUPABASE_ANON_KEY');

    if (type === 'PAYMENT_SUCCESS_WEBHOOK' && sbUrl && sbKey) {
      // Mark order as 'placed' (COD verification fee paid successfully)
      await supabaseFetch(sbUrl, sbKey, `orders?cashfree_order_id=eq.${cfOrderId}`, {
        method: 'PATCH',
        body: JSON.stringify({
          status: 'placed',
      ...
      if (resendKey) {
        ...
        try {
          const oRes = await supabaseFetch(sbUrl, sbKey, `orders?cashfree_order_id=eq.${cfOrderId}&select=id,items,total,customer_email,customer_name`);
          if (oRes.ok) {
            const rows = await oRes.json() as any[];
            if (rows.length) {
              const o = rows[0];
              const shortId = (o.id || cfOrderId).toString().slice(-8).toUpperCase();
              // Customer email
              await sendResendEmail(
                resendKey, o.customer_email || '',
                `Order Confirmed! #${shortId} — intru.in`,
                emailCodReceived(o.id || cfOrderId, o.customer_name || '', o.items || [], o.total || 0)
              ).catch((e: any) => console.error('CF customer email error:', e));
              // Manager alert
              const managerEmail = await fetchStoreSetting(sbUrl, sbKey, 'MANAGER_EMAIL') || 'shop@intru.in';
              await sendResendEmail(
                resendKey, managerEmail,
                `✅ COD VERIFIED (Cashfree) #${shortId} — Rs.${o.total}`,
                emailCodManagerAlert(o.id || cfOrderId, o.customer_name || '', '', '', o.items || [], o.total || 0)
              ).catch((e: any) => console.error('CF manager email error:', e));
            }
          }
        } catch (e) { console.error('CF webhook email error:', e); }
      }
    }

    if (type === 'PAYMENT_FAILED_WEBHOOK' && sbUrl && sbKey) {
      await supabaseFetch(sbUrl, sbKey, `orders?cashfree_order_id=eq.${cfOrderId}`, {
        method: 'PATCH',
        body: JSON.stringify({
          status: 'payment_failed',
          failure_reason: payment.payment_message || 'Cashfree payment failed',
          updated_at: new Date().toISOString(),
        }),
      });
    }
...
app.get('/api/admin/settings', async (c) => {
  ...
  return c.json({ settings: { USE_MAGIC_CHECKOUT: 'false', MANAGER_EMAIL: 'shop@intru.in', COD_FEE: '99', COD_MODE: 'manual' }, source: 'static' });
})
