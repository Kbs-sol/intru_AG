...
export function registerPage(opts: {
  ...
  const schema = JSON.stringify({
    ...
    "@type": "WebPage",
    "name": "Create Account — intru.in",
    "description": "Create your intru.in account. Get access to exclusive limited drops, order tracking, and store credit.",
    "url": "https://intru.in/register"
  });
...
.auth-perks{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:36px;padding-top:28px;border-top:1px solid var(--g100)}
.auth-perk{display:flex;align-items:start;gap:10px;padding:12px;background:var(--g50);border-radius:6px}
.auth-perk i{font-size:14px;color:var(--bk);margin-top:2px}
.auth-perk span{font-size:11px;color:var(--g500);line-height:1.5;font-weight:500}
...
@media(max-width:480px){.auth-perks{grid-template-columns:1fr}}
</style>

<div class="auth-wrap">
<div class="auth-hdr">
<p class="auth-over anim">Join the Drop</p>
<h1 class="auth-title anim d1">Create Account</h1>
<p class="auth-sub anim d2">Get access to exclusive limited drops, order tracking, and store credit.</p>
</div>

<div class="g-wrap anim d2">
<button class="g-btn" id="gSignUp" onclick="triggerGoogleSignIn()">
...
<form class="auth-form anim d3" onsubmit="event.preventDefault();sendRegisterLink()">
<input class="auth-input" type="email" id="regEmail" placeholder="Enter your email" required autocomplete="email">
<button class="auth-btn" type="submit" id="regBtn"><i class="fas fa-magic" style="margin-right:8px"></i>Send Magic Link</button>
</form>

<div class="auth-msg" id="regMsg"></div>

<p class="auth-link anim d4">Already have an account? <a href="/login">Sign in</a></p>

<div class="auth-perks anim d4">
<div class="auth-perk"><i class="fas fa-bolt"></i><span>First access to limited drops</span></div>
<div class="auth-perk"><i class="fas fa-truck"></i><span>Order tracking & history</span></div>
<div class="auth-perk"><i class="fas fa-gift"></i><span>Store credit rewards</span></div>
<div class="auth-perk"><i class="fas fa-lock"></i><span>Secure & passwordless</span></div>
</div>
</div>
...
function sendRegisterLink(){
  ...
  .then(function(d){
    if(d.success){
      msg.className='auth-msg ok';msg.textContent='Magic link sent to '+email+'. Check your inbox to complete registration!';msg.style.display='block';
  ...
  return shell(
    'Create Account — INTRU.IN | Join the Drop',
    'Create your intru.in account. Get access to exclusive limited drops, order tracking, and store credit. Sign up with Google or email magic link.',
    body,
    { url: 'https://intru.in/register', schema, razorpayKeyId: opts.razorpayKeyId, googleClientId: opts.googleClientId, products, legalPages }
  );
}
